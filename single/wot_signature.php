...
function get_part($str,$from,$to) { $value = substr($str,strpos($str,$from)+strlen($from),300); $value = substr($value,0,strpos($value,$to)); return $value; }
...
$set = file_get_contents('wot_settings.dat');
$values = explode('|',$set);
...
if (!file_exists("wot_data/backgrounds/$sig.png"))  $sig = 1;
...
/** CACHE FILENAME **/
$filename = 'wot_data/cache/'.$id.date("d_m_y").'_'.$size.'_'.$sig.'_'.$jazyk.'_'.$adv.'_'.$adv2.'_'.$set.'_'.$flag.'_'.$hsize.'.png';

/** CHECK IF ID IS ENTERED **/
if ($id=='')
{
  $img = imagecreatefrompng("wot_data/wot_404.png");
}
else if (!file_exists($filename) || !$cache)  //If this signature isn't already cached
{
  ...
  else
  {
    ...
    **/
    ...
    $img = imagecreatefrompng("wot_data/wot_empty_$size.png");
    ...
    //Load background
    $bg = imagecreatefrompng("wot_data/backgrounds/$sig.png");
    //This image is used to dark image
    $tm = imagecreatefrompng("wot_data/wot_black.png");
    ...
    switch($size)
    {
      case 0: $titlesize = 14; $fontsize = 8; $font = "wot_data/fonts/CALIBRI.TTF"; break;
      case 1: $titlesize = 16; $fontsize = 10; $font = "wot_data/fonts/CALIBRI.TTF"; break;
    }
    ...
    $fontB = "wot_data/fonts/ARIALBD.TTF";
    $box = imagettfbbox($titlesize, 0, $fontB, $login);
    $xx = 8;
    
    //Draw flag
    if ($flag!="" && file_exists("wot_data/flags/$flag.gif"))
    {
      $flagi = imagecreatefromgif("wot_data/flags/$flag.gif");
    ...
    **/
      ...
      foreach($tanky as $tank)
      {
        ...
        if ($xx + $tankwidth + 4 < $width)
        {
          ...
          if ($tanksize == 0)
          {
            ...
            $bg = imagecreatefrompng("wot_data/imgSmall/$tank[icon]");
    ...
    if ($cache)
    {
      ...
      if (!file_exists('wot_data/cache/cache'.date('d_m_y')))
      {
        $d = opendir('wot_data/cache/');
        while($s = readdir($d))
        {
          if ($s!='.' && $s!='..')
            if (strpos($s,date('d_m_y')) == 0)
              unlink("wot_data/cache/$s");
        }
        $f = fopen('wot_data/cache/cache'.date('d_m_y'),'w');
        fclose($f);
      }
    }
  }
